...
(function() {
    ...
    function initToastEngine(toastsData) {
        ...
        function trackToastEvent(action, data) {
            ...
            // 3. Salva contador interno no WordPress (para views e clicks)
            if ((action === 'view' || action === 'click') && apiBase) {
                const trackUrl = apiBase.replace(/\/$/, '') + '/wp-json/abc-toast/v1/track';
                const payload = JSON.stringify({ toast_id: data.id, action: action });
                let sent = false;
